...
function buildWelcomeEmail(fullName: string, email: string, password: string, role: string) {
  const loginUrl = `${process.env.NEXT_PUBLIC_APP_URL || ""}/login`;
  const roleLabel = role === "ADMIN" ? "Admin" : "Clinician";

  return {
    subject: `Your ${BRANDING.shortName} ${roleLabel} account has been created`,
    html: `
      <!DOCTYPE html>
      <html lang="en">
                ...
                <tr>
                  <td style="background:#ffffff;padding:36px;border-left:1px solid #e9eef5;border-right:1px solid #e9eef5;">
                    ...
                    <p style="margin:0 0 24px;font-size:15px;line-height:1.7;color:#475569;">
                      Your <strong>${roleLabel}</strong> account on the <strong>${BRANDING.shortName}</strong>
                      portal has been created. Use the credentials below to sign in.
                    </p>
                      ...
                      <tr>
                        <td style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:20px 24px;">
                          ...
                          <table cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                              <td style="padding:6px 0;font-size:14px;color:#64748b;width:90px;">Email</td>
                    ...
                    <p style="margin:0 0 20px;font-size:14px;color:#f59e0b;font-weight:600;">
  ...
  try {
    ...
    const result = await db.query(
      ...
      RETURNING
        ...
        updated_at
      `,
      [fullName, null, email, passwordHash, role, isActive, true]
    );

    // Send welcome email with credentials — non-blocking
    const emailTemplate = buildWelcomeEmail(fullName, email, password, role);
    resend.emails.send({
